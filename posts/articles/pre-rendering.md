...
date: '2022-01-01'
excerpt: 'Building resilient services by identifying foreign state mutations and grouping local changes into restartable atomic phases so that every request can be driven to completion.'
---

In APIs **_idempotency_** is a powerful concept. An
...
to introduce **_idempotency keys_**.
...
{{Figure "Rocket Rides the app." (ImgSrcAndAltAndClass "/assets/images/idempotency-keys/rocketrides-ios-ride.png" "Rocket Rides the app." "overflowing")}}
...
{{Figure "A typical API request to our embellished Rocket Rides backend." (ImgSrcAndAltAndClass "/assets/images/idempotency-keys/api-request.svg" "A typical API request to our embellished Rocket Rides backend." "overflowing")}}
...
making **_foreign state mutations_**; that is, calling out
...
An **_atomic phase_** is a set of local state mutations
...
A **_recovery point_** is a name of a check point that we
...
3. After those phases have been identified, **_all other
   operations between_** them are grouped into atomic
...
{{Figure "API request to Rocket Rides broken into foreign state mutations and atomic phases." (ImgSrcAndAltAndClass "/assets/images/idempotency-keys/atomic-phases.svg" "API request to Rocket Rides broken into foreign state mutations and atomic phases." "overflowing")}}
...
In the case of a serialization error, we return a `409 Conflict` because that almost certainly means that a
...
operation. If the key was already locked, return a `409 Conflict` to indicate that to the user.
...
There should be an **_enqueuer_** that moves jobs from
...
A stretch goal is to implement a **_completer_**. Its only
...
**_reaper_** process should go through keys and delete
them.
...
- _The client makes a request, but the connection breaks
  before it reaches the backend:_ The client, having used
  an idempotency key, knows that retries are safe and so
  retries. The next attempt succeeds.

- _Two requests try to create an idempotency key at the
  same time:_ A `UNIQUE` constraint in the database
  guarantees that only one request can succeed. One goes
  through, and the other gets a `409 Conflict`.

- _An idempotency key is created, but the database goes
  down and it fails soon after:_ The client continues to
  retry against the API until it comes back online. Once it
  does, the created key is recovered and the request is
  continued.

- _Stripe is down:_ The atomic phase containing the Stripe
...
- _A server process dies while waiting for a response from
  Stripe:_ Luckily, the call to Stripe was also made with
...
- _A bad deploy 500s all requests midway through:_
...
When rendering the form initially, we can add a `<input type="hidden">` to it that contains an idempotency key.
...
API backends should aim to be _passively safe_ -- no matter
